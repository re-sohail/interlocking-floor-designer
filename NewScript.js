...
function updateDimensions() {
  ...
  const ftWidth = (width / pixelsPerFoot).toFixed(1);
  const ftHeight = (height / pixelsPerFoot).toFixed(1);

  // Update main dimension labels
  ...
  const areaFt = ((width * height) / (pixelsPerFoot * pixelsPerFoot)).toFixed(
    2
  );
  dimensionsDisplay.textContent = `Width: ${ftWidth}ft | Height: ${ftHeight}ft | Area: ${areaFt} ft²`;
}
